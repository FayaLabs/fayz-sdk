-- ============================================================================
-- @fayz-ai/shop — the charged freight matches the freight the cart showed
-- ----------------------------------------------------------------------------
-- 0015 moved shipping to the server and computed it on the post-discount
-- subtotal. The cart computes it on the PRE-discount subtotal, so an order of
-- R$ 159.98 with a 10% coupon showed "frete grátis / total 143.98" and was
-- charged 151.98. Caught by the journey e2e comparing the confirmation total to
-- the cart total.
--
-- Aligning the server to the storefront rather than the other way round: making
-- a coupon cost the buyer their free delivery is a pricing decision for the
-- merchant to take deliberately, not a side effect of a migration.
-- ============================================================================
...
BEGIN
  ...
  -- cart store (packages/storefront/src/stores/cart.store.ts:150-158). This is
  -- not a detail: computing it post-discount made the cart advertise free
  -- delivery and the server charge R$ 8 on the same order — the customer saw
  -- 143.98 and was billed 151.98. Whether a coupon should cost the buyer their
  -- free shipping is a business decision; until it is made, the number the
  -- storefront promises is the number that gets charged.
  -- A free_shipping discount still wins (it set v_free_shipping above).
  IF NOT v_free_shipping THEN
    v_shipping := COALESCE(public.shop_shipping_for(p_tenant_id, v_subtotal), 0);
  END IF;
