-- ============================================================================
-- @fayz-ai/shop — the payment ledger points at the order by FK, not by metadata
-- ----------------------------------------------------------------------------
-- 0012 stashed the order id in transactions.metadata because core orders did
-- not exist yet: transactions.order_id references public.orders, and a shop
-- order was not one. 0013 changed that — every shop order now has a core row
-- with the SAME id, created by a BEFORE INSERT trigger, so by the time
-- shop_place_order writes the ledger the FK target already exists.
--
-- Leaving the link in jsonb costs a real join: finance cannot ask "payments for
-- this order" through a foreign key, and nothing protects the reference.
--
-- The function is re-emitted in full rather than patched in place. Rewriting a
-- body by string replacement at migration time fails silently when the source
-- differs by a character, which is worse than being verbose.
--
...
-- Settlement closes the ledger by FK, keeping the metadata path as a fallback
