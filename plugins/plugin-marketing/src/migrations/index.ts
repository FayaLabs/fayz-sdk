// Inline copy of 001_content_planner.sql for manifest.migrations[] (the
// forward-compat install-lifecycle delivery). The .sql file next to this is
// what beauty-saas' db-apply actually executes today — keep both in sync.
...
// Inline copy of 002_multi_platform.sql — same keep-in-sync contract as 001.
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'mkt_social_accounts' AND column_name = 'platform'
  ) THEN
    UPDATE public.mkt_social_accounts
      SET platforms = ARRAY[platform]
      WHERE platform IS NOT NULL AND platforms = '{instagram}';
    ALTER TABLE public.mkt_social_accounts DROP COLUMN platform;
  END IF;
END $$;
...
// Inline copy of 003_recording_ops.sql — same keep-in-sync contract as 001.
...
DROP POLICY IF EXISTS mkt_media_read ON storage.objects;
DROP POLICY IF EXISTS mkt_media_insert ON storage.objects;
DROP POLICY IF EXISTS mkt_media_update ON storage.objects;
DROP POLICY IF EXISTS mkt_media_delete ON storage.objects;
CREATE POLICY mkt_media_read ON storage.objects FOR SELECT TO public USING (bucket_id = 'mkt-media');
CREATE POLICY mkt_media_insert ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id = 'mkt-media');
CREATE POLICY mkt_media_update ON storage.objects FOR UPDATE TO authenticated USING (bucket_id = 'mkt-media');
CREATE POLICY mkt_media_delete ON storage.objects FOR DELETE TO authenticated USING (bucket_id = 'mkt-media');`
