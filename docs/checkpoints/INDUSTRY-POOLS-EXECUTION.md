...
- [x] M4 PUBLISH WAVE FEITO (2026-07-15, founder autorizou expressamente): TODOS os 33 pacotes @fayz-ai publicados como 0.8.0 unificado (linha estável industry-pools, acima de todas as linhas anteriores; deps internas ^0.8.0 via workspace:^; resolução smoke-testada; release-channels sincronizados). Original:  (db/core majors + saas + touched plugins + shop + courses + cli); apps flip tarball→ranges
...
- 2026-07-15 · M6 FECHADO. Rollout 0.8.0: 8/8 apps em ^0.8.0 (npm), single core, overrides removidos, hooks doctor instalados (prepare+hooksPath). Acabamento: manifests dos booking sites com component declarado (custom:BookingScreen/BlogList/PostDetail/ReviewsScreen) → doctor 0 ERRORS nos 8; lockfiles únicos (pnpm/yarn obsoletos removidos); plugin-agenda 0.8.1 publicado (declara @fullcalendar/resource que faltava — beauty build:published-sdk verde); espaco: webhook test rodava com ts-node quebrado (pré-existente) → tsx, gate pre-push --full passa honesto. Docs Dev Center: página Testar-e-debugar atualizada (deps/hygiene/--full) no branch local docs/doctor-08-checks do fayz-docs (repo sem remote — founder mergeia). PRs todos atualizados.

- 2026-07-15 · M6 release: fayz doctor estendido (f76b343: deps file:/tgz=ERROR + --remote, hygiene .env/lockfile, --full build+test; 76 testes cli). Bump unificado 0.8.0 nos 34 packages + publish via worktree (changeset) → npm view confirma 33/33 em 0.8.0. Rollout agent flipando os 8 apps file:→^0.8.0 + hooks doctor (pre-commit=doctor, pre-push=doctor --full) + PRs.

- 2026-07-15 · M6 em curso (feedback do founder nos PRs): (a) .env untracked nos 3 repos que trackeavam (hempdent/great-djs/espaco; anon-key real achada e removida do .env.example do espaco); (b) contracts padronizados (5 commits: T registry em todos os registries, exports ./schema do marketing, checker de idempotência repo-wide 62 arquivos/11 dirs, docs/PLUGIN-CONTRACT.md, triggers replay-safe); CORREÇÃO CRÍTICA por cima: agent editou shop/0001+courses/0001 JÁ APLICADAS (drift hard-stop garantido) → bytes revertidos pra bater com os ledgers (verificado) + allowlist congelada no checker; (c) fila: fayz doctor estendido (deps file:/tgz=error, hygiene .env/lockfile, --full build+test — coerente com Dev Center testar-e-debugar) → bump unificado 0.8.0 → npm publish → apps flipam file:→^0.8.0 + hooks doctor → PRs atualizados.

- 2026-07-15 · PRs ABERTOS em todos os repos pra validação do founder (builds verdes em todos): fayz-sdk#15, ymaia#1011 (registry, base dev), beauty-saas#7, resto-saas#4, agency-os#1, artorious-shop#1, marketplace-saas#1 (WIP topbar→sidebar intocado local), hempdent#4, great-djs-school#4 e espaco-renova-rio#7 (esses dois no org MaiaVinicius). course-admin fora (WIP Vini). Verificação pré-PR: sdk build 34/34 + typecheck + 106 testes + idempotency checker; 8 apps build de produção PASS.

- 2026-07-15 · M5 FECHADO: agency-os real-mode PASS — contato "Smoke Contact M5" (101aa5c8) criado via POST /people 201 e persistente após reload; ambos os bugs reportados pelo founder provados corrigidos e2e. Matriz final: beauty auth+read ✓ (5301), resto auth+create ✓ (5302), agency auth+create ✓ (5303), artorious checkout e2e ✓ (5304), marketplace auth+read ✓/create sem UI (5305), hempdent booking ✓ (5307), great-djs booking ✓ (5308), espaco booking ✓ (5309). Loop encerrado; restam apenas gates do founder (M4 publish, registry push/migrate, merge dos branches) + follow-ups documentados.

- 2026-07-15 · M5 round 3: artorious checkout e2e PASS ("Pedido confirmado!" #6, Pago, R$77,99) — cadeia completa: shop_resolve_customer → shop_place_order → shop_get_order fallback → shop_confirm_payment (novo seam RPC 0006/0006b: só pending→paid de pedido open, uuid=capability; provider.confirmPayment opcional preferido no checkout workflow). 401s residuais no console = leitura direta pré-fallback (cosmético; polish futuro: anon tenta RPC primeiro). Seed: teste@teste.com owner no tenant Agency Demo; verificação final do create-contact real-mode do agency dispatched.
