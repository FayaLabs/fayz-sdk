...
- **Migration bench** — `packages/shop/test/run-migrations.sh`: applies the shop chain (plus the financial schema it bridges into) to a throwaway Postgres, runs eight SQL behaviour regressions against it, then **replays the whole chain twice** and asserts the second pass moves nothing. The static checker (`check-idempotent-migrations.mjs`) catches non-replay-safe DDL *shapes*; only the bench catches the rest — a `CREATE OR REPLACE FUNCTION` that cannot widen its own OUT columns, a temp-table snapshot that commits itself away, a backfill that re-inserts. This is the retro-compatibility gate for every already-provisioned pool: live apps run against databases where these files have already been applied once, and `fayz db apply` re-runs a file the moment its checksum moves.
