...
Last updated: 2026-06-14 19:10 BRT
...
24. M57 moved `marketplace-saas` to the same manifest-first app contract as Beauty/Resto: tiny `src/App.tsx`, `src/config/app.tsx`, `defineSaas(config)`, and `renderApp(manifest, { surface: 'admin' })`. Build and browser smoke are green on `http://localhost:5186/`. Next marketplace step is further config split plus SDK-backed shop/admin data proof.
