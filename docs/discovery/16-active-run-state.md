...
Last updated: 2026-06-14 20:17 BRT
...
12d. M71 adds `createFayzMenuProvider()` inside private `@fayz-ai/plugin-menu`, backed by `@fayz-ai/sdk` data reads/mutations for `saas_core.categories` and `saas_core.products` with menu-specific metadata. Resto still needs env-gated Menu wiring before real provider smoke.
