...
Last updated: 2026-06-14 16:07 BRT
...
14. Storefront proof: Shopfront, Tannat, and Pulse now prove the right direction: catalog/checkout/profile/header/footer are platform-owned in `packages/storefront`, while each store owns config, theme, catalog, copy, images, and business-specific customization. M38 removed the default storefront path that created Supabase clients and added narrow `@fayz-ai/shop/runtime`, `@fayz-ai/shop/mock`, and `@fayz-ai/shop/catalog` subpaths. M39 added the first code-level customization slot (`ProductCard`) and dogfooded it in Pulse without copying catalog/checkout/account pages. Next step is expanding slots only where real app customization pressure proves they are needed.
