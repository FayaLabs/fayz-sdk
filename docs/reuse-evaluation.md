...
Written 2026-06-11, after building three storefronts on `@fayz/storefront`:
...
- **Shared engine: ~3,200 lines** in `@fayz/storefront` + `@fayz/shop`/`@fayz/ui`/`@fayz/auth`/`@fayz/core`.
...
1. **Auth** — the storefront customer session runs on the *same* `AuthAdapter` contract
   (`@fayz/core`) and the *same* adapters (`createMockAuthAdapter`, `createSupabaseAuthAdapter`
   from `@fayz/auth`) that `createSaasApp` uses for admin login. The storefront adds exactly one
...
2. **Commerce data** — one `ShopProvider` contract serves the merchant admin
   (`@fayz/plugin-shop` inside marketplace-saas) AND all customer storefronts. Same orders,
   same tables, mock or Supabase.
3. **Design tokens** — `StorefrontTheme` writes the same CSS custom properties
   (`--primary`, `--background`, `--button-radius`, `--font-family`) that `@fayz/ui`'s preset and
...
1. **SaasTheme vs StorefrontTheme are parallel types, not one type.** Same convention,
   duplicated concept. Fix: extract a shared `ThemeTokens` base into `@fayz/ui` that both extend.
2. **Build-config skeletons are copy-pasted** (vite.config/tsconfig/tailwind ×4 apps now).
   Drift risk is real — the port is the only intentional difference. Fix: `@fayz/cli create-store`
...
3. **Storefront sections hand-roll buttons/inputs with tailwind classes instead of using
   `@fayz/ui` Button/Input.** Justified short-term (admin primitives carry admin styling), but the
...
4. **Mock auth adapter's `signIn()` ignores the email** (always logs in the configured user);
   ...
   `@fayz/auth`: identity-bearing mock sign-in.
